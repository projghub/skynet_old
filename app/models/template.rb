class Template < ActiveRecord::Base
	...
	def select_ads(publisher, attr_filter)
		q = Ad.joins(:ad_group => {:campaign => :account}).joins("left join ads_publishers on ads_publishers.ad_id = ads.id").where('ads.enabled = ? and campaigns.enabled = ? and accounts.enabled = ? and ads.ad_type_id = ?', true, true, true, self.ad_type_id).order("ads.bid desc, rand()").where('ads_publishers.publisher_id = ? or ads_publishers.publisher_id is null', publisher.id).limit(self.positions)
