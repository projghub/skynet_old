class TemplatesController < AuthenticateController
	# GET /templates
	# GET /templates.xml
	def index
	...
	def preview
		@template = Template.find(params[:id])
		@ads = [Ad.includes({:ad_group => :campaign}, :ad_type).where("campaigns.account_id = ?", @auth_user.account_id).find(params[:ad_id])]
		render @template.file, :layout => "serve"
	end
